...
public class MongoJiraTicketServiceImpl implements MongoJiraTicketService{
    ...
    @Value("${com.example.stacktradeapp.mongodb.collection.name}")
    ...
    public BasicDBObject getTicketById(String id) throws NotFoundException {
        ...
        return mongoTemplate.findOne(query, BasicDBObject.class, mongoCollectionName);
    }
    ...
    public Collection<Document> insertTickets(List<Document> tickets) {
        Collection<Document> inserted =  mongoTemplate.insert(tickets, mongoCollectionName);
        logger.info("inserted " + tickets.size() + "tickets to MongoDB");
        return inserted;
    }
